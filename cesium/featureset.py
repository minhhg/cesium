...
# TODO handle reading from NetCDF: restore proper coordinate structure
# (cf. output of `assemble_featureset` w/ loaded version)
class Featureset(xr.Dataset):
    ...
    def impute(self, strategy='constant', value=None, axis=0):
        ...
        axis : integer, optional
            Axis along which to perform imputation, if applicable. `axis=0`
            imputes along columns, `axis=1` along rows. Defaults to 0.
    ...
    def __getitem__(self, key):
        ...
        elif (key in names or (hasattr(key, '__iter__') and all(el in names for
                                                                el in key))):
